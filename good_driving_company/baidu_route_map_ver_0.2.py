...
def route_plot(data, map_center):
            ...
            for i in range(len(time_cut_index)):
                ...
                if len(trip_temp_data) > 50:
                    bd_map = bd_map.add(
                        ...
                        type_="lines",
                        is_polyline=True,
                        data_pair=[route_to_baidu_line(trip_temp_data, color='green')],
    ...
    # 设置图例
    bd_map = bd_map.set_global_opts(legend_opts=opts.LegendOpts(is_show=False,  # 不显示标签
                                                                ),
...
if __name__ == "__main__":
    ...
    bd_plot = route_plot(data=all_data,
    ...
    for j in range(len(risk_route)):
        ...
        bd_plot.add_coordinate(name=name,
                               longitude=longitude,
                               latitude=latitude
                               )
    ...
    # 将绘图结果输出至网页文件
    bd_plot = bd_plot.add_control_panel(maptype_control_opts=opts.BMapTypeControlOpts(position=1),
                                        )

    # 将地图加载到网页
    bd_plot.render(lines_file_dir)
