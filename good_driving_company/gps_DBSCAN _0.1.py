...
def plot_map(map_type, point_data):
    ...
    # 创建异常驾驶行为的FeatureGroup
    abnormal_group = folium.FeatureGroup(name='异常行为分布', control=True)
    ...
    for behavior in behavior_list:
        ...
        for name, row in behavior_data.iterrows():
            ...
            # iframe = folium.IFrame(html=heading3(parse_zhch(behavior)), width=300, height=50)
    ...
    for label in label_list:
        if label == -1:
            ...
            folium.Circle(location=[label_data['lat'].iloc[i],
                                    label_data['long'].iloc[i]],
                          radius=500,
                          # popup='popup',
                          color='gray',
                          fill=True,
    ...
    map_plot.add_child(abnormal_group)
...
def main():
    ...
    # 每500米有5次异常行为集中出现
    labels = DBSCAN(eps=500, min_samples=5, metric='precomputed').fit_predict(distance_matrix)
    '''
